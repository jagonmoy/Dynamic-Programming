...
ll booleanParenthesize(ll i , ll j , bool fact) {
    if ( i > j) return dp[i][j][fact] = 0 ;
    if ( i == j) {
        if(fact) {
            if ( s[i] == 'T') return dp[i][j][fact] = 1 ;
            else return dp[i][j][fact] = 0 ;
        }
        else {
            if ( s[i] == 'F') return dp[i][j][fact] = 1 ;
            else return dp[i][j][fact] = 0 ;
        }
    }
    if ( dp[i][j][fact] != -1) return dp[i][j][fact];
    ll ans = 0 ;
