...
ll minimumCoins(ll index , ll capacity) {
    ...
    if ( index == 0) return (inf-1);
    if ( CoinArray[index] <= capacity) {
        return  min(minimumCoins(index,capacity-CoinArray[index])+1,minimumCoins(index-1,capacity));
    } 
    else return minimumCoins(index-1,capacity);
}
int main() {
