...
int lcsvariation(int i , int j , int repeat) {
    ...
    if (!repeat) {
        ...
        else return dp[i][j][repeat] = max({lcsvariation(i+1,j,0),lcsvariation(i,j+1,0)}); 
    }
    else {
        if (s1[i] == s2[j]) return dp[i][j][repeat] = max({lcsvariation(i+1,j,1)-1,lcsvariation(i,j+1,1)-1,2+lcsvariation(i+1,j+1,1),2});
